...
func (db *DB) Select(id int) (Data, error) {
	var DataQuery Data
	ErrorQuery := db.QueryRow("SELECT * FROM bazarakiLis WHERE ID = ?", id).Scan(&DataQuery.ID, &DataQuery.Name, &DataQuery.Link, &DataQuery.Area, &DataQuery.Price, &DataQuery.TimeCreate)
	if ErrorQuery != nil {
...
func (db *DB) Selects() ([]Data, error) {
	...
	for rows.Next() {
		var DataQuery Data
		if err := rows.Scan(&DataQuery.ID, &DataQuery.Name, &DataQuery.Link, &DataQuery.Area, &DataQuery.Price, &DataQuery.TimeCreate); err != nil {
